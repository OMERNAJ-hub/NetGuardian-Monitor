# Copilot Instructions for NetGuardian

Warning: Only maintainers of NetGuardian can use this instructions, for other contributors, must read AGENTS.md and CLAUDE.md to avoid to get banned because of AI slop.
...
**NetGuardian** is a self-hosted monitoring tool for HTTP(s), TCP, DNS, Docker, etc. Built with Vue 3 (frontend) and Node.js/Express (backend), using Socket.IO for real-time communication.
...
│   └── netguardian-server.js  Main server logic
...
2. `server/netguardian-server.js` (register monitor type)
