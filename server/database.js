...
class Database {
    ...
    static async patch(port = undefined, hostname = undefined) {
        // Still need to keep this for old versions of NetGuardian
        ...
        try {
            ...
            if (e.message.includes("the following files are missing:")) {
                ...
                log.warn("db", "Database migration failed, you may be downgrading NetGuardian.");
            } else {
    ...
    static async patchSqlite() {
        ...
        if (version === this.latestVersion) {
            ...
            try {
                ...
                log.error("db", ex);
                log.error("db", "Start netguardian failed due to issue patching the database");
                log.error(
                    "db",
                    "Please submit a bug report if you still encounter the problem after restart: https://github.com/louislam/netguardian/issues"
                );
    ...
    static async patchSqlite2() {
        ...
        try {
            ...
            log.error(
                "db",
                "Please submit the bug report if you still encounter the problem after restart: https://github.com/louislam/netguardian/issues"
            );
